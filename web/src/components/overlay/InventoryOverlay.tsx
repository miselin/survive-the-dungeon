...
function buildInventoryComparison(
  ...
  const rows: ComparisonRow[] = [
    {
      label: EN.ui.compare.atkBonus,
      currentValue: signed(currentAtk),
      newValue: signed(newAtk),
      deltaValue: newAtk - currentAtk,
    },
    {
      label: EN.ui.compare.defBonus,
      currentValue: signed(currentDef),
      newValue: signed(newDef),
      deltaValue: newDef - currentDef,
    },
  ];
  ...
  if (nextWeapon || currentWeapon) {
    const currentDice =
      currentWeapon?.damage() ?? EN.ui.compare.empty.toLowerCase();
    ...
    rows.push({
      ...
      currentValue: currentWeapon
        ? `${currentWeapon.criticalRange()}`
        : EN.ui.compare.empty.toLowerCase(),
      newValue: nextWeapon
        ? `${nextWeapon.criticalRange()}`
        : EN.ui.compare.empty.toLowerCase(),
      deltaValue:
        (nextWeapon?.criticalRange() ?? 20) -
        (currentWeapon?.criticalRange() ?? 20),
      preferLower: true,
    });
    rows.push({
      label: EN.ui.compare.critMult,
      currentValue: currentWeapon
        ? `${currentWeapon.criticalMultiplier()}`
        : EN.ui.compare.empty.toLowerCase(),
      newValue: nextWeapon
        ? `${nextWeapon.criticalMultiplier()}`
        : EN.ui.compare.empty.toLowerCase(),
      deltaValue:
        (nextWeapon?.criticalMultiplier() ?? 1) -
        (currentWeapon?.criticalMultiplier() ?? 1),
    });
  }
...
function InventoryComparePanel(props: {
  run: DungeonRun;
  itemId: string | null;
}) {
  if (!props.itemId) {
  ...
  return (
    <>
      <div className="compare-title">
        ...
        <span>
          {EN.ui.compare.slot}: {comparison.slotName}
        </span>
        <span>
          {EN.ui.compare.current}: {comparison.currentName}
        </span>
      </div>
      <table className="compare-table">
        ...
        <tbody>
          {comparison.rows.map((row) => (
            <tr
              key={`${row.label}-${row.currentValue}-${row.newValue}-${row.deltaValue}`}
            >
              <td>{row.label}</td>
...
type InventoryOverlayProps = {
  ...
  onCompareItemChange: (itemId: string | null) => void;
  onClose: () => void;
...
export function InventoryOverlay(props: InventoryOverlayProps) {
  ...
  const handleEquipFocus = (itemId: string): void => {
    props.onCompareItemChange(itemId);
  };

  const handleEquipBlur = (event: FocusEvent<HTMLButtonElement>): void => {
    const next = event.relatedTarget;
    if (!(next instanceof HTMLElement)) {
      props.onCompareItemChange(null);
      return;
    }

    const nextEquipButton = next.closest<HTMLButtonElement>(
      "button[data-compare-equip='true']",
    );
    if (!nextEquipButton) {
      props.onCompareItemChange(null);
    }
            ...
            rows.map(({ item, action }) => (
              <li key={item.id}>
                ...
                <div className="item-actions">
                  {action === "equip" ? (
                    <button
                      data-compare-equip="true"
                      type="button"
                      onClick={() => {
                        props.onEquipItem(item.id);
                      }}
                      onMouseEnter={() => {
                        props.onCompareItemChange(item.id);
                      }}
                      onMouseLeave={() => {
                        props.onCompareItemChange(null);
                      }}
                      onFocus={() => {
                        handleEquipFocus(item.id);
                      }}
                      onBlur={handleEquipBlur}
                    >
                      {EN.ui.buttons.equip}
                    </button>
                  ) : action === "use" ? (
