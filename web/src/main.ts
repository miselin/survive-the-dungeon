...
import { DungeonRun, type BuildChoice } from "./game";
...
declare global {
  interface Window {
    __surviveDebug?: {
      ...
      moveTo: (x: number, y: number, opts?: { ignoreMobs?: boolean; allowMobId?: string; ignoreChests?: boolean }) => DebugMoveResult;
      ...
      allocateLevelUp: (attr: "str" | "dex" | "con" | "int" | "wis" | "chr") => void;
      canOpenShop: () => boolean;
...
function mountRun(activeRun: DungeonRun): void {
  ...
  modal.addEventListener("click", (event) => {
    ...
    if (action === "combat-normal") {
      ...
      run.allocateLevelUp(id as "str" | "dex" | "con" | "int" | "wis" | "chr");
...
function debugMoveResult(activeRun: DungeonRun, steps: number, ok: boolean, reason?: string): DebugMoveResult {
  return {
    ok,
    steps,
...
function installDebugBridge(): void {
  ...
  const moveTo = (
    ...
    target: Position,
    opts?: { ignoreMobs?: boolean; allowMobId?: string; ignoreChests?: boolean },
  ): DebugMoveResult => {
    if (activeRun.overlay.type !== "none") {
  ...
  window.__surviveDebug = {
    ...
    canOpenShop: (): boolean => (run ? run.canOpenShop() : false),
    grantRegressionPower: (): void => {
      if (!run) {
        return;
      }
    ...
    importSaveToken: (token: string): boolean => tryLoadRunFromTokenInput(token),
  };
}
