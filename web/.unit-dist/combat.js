import { COMBAT_DEFENSIVE_ATTACK_MULTIPLIER, COMBAT_DEFENSIVE_DEFENSE_MULTIPLIER, COMBAT_OFFENSIVE_ATTACK_MULTIPLIER, COMBAT_OFFENSIVE_DEFENSE_MULTIPLIER, ENEMY_STYLE_GUARDED_ATTACK_MULTIPLIER, ENEMY_STYLE_GUARDED_CHANCE_PERCENT, ENEMY_STYLE_GUARDED_DEFENSE_MULTIPLIER, ENEMY_STYLE_LOW_HP_RATIO, ENEMY_STYLE_RECKLESS_ATTACK_MULTIPLIER, ENEMY_STYLE_RECKLESS_CHANCE_PERCENT, ENEMY_STYLE_RECKLESS_DEFENSE_MULTIPLIER, ENEMY_STYLE_STEADY_ATTACK_MULTIPLIER, ENEMY_STYLE_STEADY_DEFENSE_MULTIPLIER, FLEE_BASE_DC, FLEE_DIE_SIDES, MAXIMUM_INEFFECTIVE_DAMAGE_MULTIPLIER, PLAYER_LUCK_BONUS_PER_LOW_ROLL, PLAYER_LUCK_BONUS_PER_MISS_STREAK, PLAYER_LUCK_CRITICAL_HP_EXTRA_BONUS, PLAYER_LUCK_CRITICAL_HP_RATIO, PLAYER_LUCK_HISTORY_SIZE, PLAYER_LUCK_LOW_HP_BONUS, PLAYER_LUCK_LOW_HP_RATIO, PLAYER_LUCK_LOW_ROLL_THRESHOLD, PLAYER_LUCK_LOW_ROLL_TRIGGER, PLAYER_LUCK_MAX_HISTORY_BONUS, PLAYER_LUCK_MAX_MISS_STREAK_BONUS, PLAYER_LUCK_MAX_TOTAL_BONUS, } from "./constants";
import { Creature } from "./creature";
import { Dice } from "./dice";
import { InstantEffectItem } from "./items";
import { EN } from "./strings/en";
export function armorClassFor(defender, defenseMultiplier = 1) {
    return Math.ceil(10 + (defender.defenseBonus * defenseMultiplier) + defender.attributes.modifier("dex"));
}
export function playerLuckBonusFromState(state, hpRatio) {
    ...
    const historyBonus = Math.min(PLAYER_LUCK_MAX_HISTORY_BONUS, triggerExcess * PLAYER_LUCK_BONUS_PER_LOW_ROLL);
    const missStreakBonus = Math.min(PLAYER_LUCK_MAX_MISS_STREAK_BONUS, state.playerMissStreak * PLAYER_LUCK_BONUS_PER_MISS_STREAK);
    let hpBonus = 0;
...
export class Combat {
    ...
    turn(player, enemy, action, options = {}) {
        ...
        if (enemy.alive) {
            ...
            this.attack(enemy, player, enemyStyle.atk * Math.max(0, options.enemyAttackMultiplier ?? 1), playerDefMult * enemyStyle.def, logs, moments);
        }
        player.think();
        enemy.think();
...
export function canPlayerHeal(player) {
    return player.healItems().length > 0;
}
export function describeHealChoice(player) {
    const item = player.bestHealItem();
    if (!item) {
