...
import { buildSaveUrl, canUseShareableUrl, encodeSaveToken, extractSaveToken, extractSaveTokenFromSearch, loadRunFromToken, readLatestSaveToken, storeLatestSaveToken, } from "./save";
...
function signedFixed(num, precision) {
...
function parseDiceAverage(dice) {
...
function comparisonRow(label, currentValue, newValue, deltaValue, preferLower = false, deltaPrecision = 0) {
    return `
    <tr>
...
function emptyComparisonPanel() {
    return `<p class="hint">${htmlEscape(EN.ui.compare.hoverHint)}</p>`;
}
function renderBuildChoices(choices, action) {
    return choices
        .map((choice) => `
      <li>
        <div>
...
function queueCombatFx(moments) {
    if (moments.length === 0) {
        return;
    }
    combatFx = {
...
function advanceCombatFx(deltaMs) {
...
function renderCombatMoment(moment) {
    if (moment.type === "roll") {
...
function buildEquipComparison(activeRun, itemId) {
    ...
    if (nextWeapon || currentWeapon) {
        ...
        rows.push(comparisonRow(EN.ui.compare.critRangeStart, currentWeapon ? `${currentWeapon.criticalRange()}` : EN.ui.compare.empty.toLowerCase(), nextWeapon ? `${nextWeapon.criticalRange()}` : EN.ui.compare.empty.toLowerCase(), (nextWeapon?.criticalRange() ?? 20) - (currentWeapon?.criticalRange() ?? 20), true));
        rows.push(comparisonRow(EN.ui.compare.critMult, currentWeapon ? `${currentWeapon.criticalMultiplier()}` : EN.ui.compare.empty.toLowerCase(), nextWeapon ? `${nextWeapon.criticalMultiplier()}` : EN.ui.compare.empty.toLowerCase(), (nextWeapon?.criticalMultiplier() ?? 1) - (currentWeapon?.criticalMultiplier() ?? 1)));
    }
    return `
...
function updateInventoryCompare(itemId) {
    if (!run || !ui || run.overlay.type !== "inventory") {
        return;
    }
    const target = ui.modal.querySelector("[data-equip-compare]");
    if (!target) {
        return;
    }
...
function persistRunSave(activeRun) {
    const token = encodeSaveToken(activeRun);
    storeLatestSaveToken(token);
    return token;
}
async function saveRunTokenAndShare() {
    if (!run) {
        return;
    }
    ...
    if (copied) {
        window.alert(useShareUrl ? EN.ui.saves.shareUrlCopied : EN.ui.saves.tokenCopied);
        return;
    }
    window.prompt(useShareUrl ? EN.ui.saves.copyShareUrlPrompt : EN.ui.saves.copyTokenPrompt, primaryValue);
}
function tryLoadRunFromTokenInput(rawValue) {
...
function promptForSaveTokenAndLoad() {
...
function showMenu() {
  ...
  `;
    const start = app.querySelector("#start-run");
    const load = app.querySelector("#load-run");
    const resumeLatest = app.querySelector("#resume-latest");
    const input = app.querySelector("#seed-input");
...
function mountRun(activeRun) {
  ...
  `;
    ...
    const logs = app.querySelector("#logs");
    const seedValue = app.querySelector("#seed-value");
    const stateText = app.querySelector("#state-text");
    const saveButton = app.querySelector("#save-btn");
    const loadButton = app.querySelector("#load-btn");
    const shopButton = app.querySelector("#shop-btn");
    const inventoryButton = app.querySelector("#inventory-btn");
    const revealButton = app.querySelector("#reveal-map-btn");
    const dpad = app.querySelector("#dpad");
    const modalBackdrop = app.querySelector("#modal-backdrop");
    const modal = app.querySelector("#modal");
    const newRun = app.querySelector("#new-run-btn");
    if (!root
        || !canvas
        || !stats
        || !logs
    ...
    modal.addEventListener("click", (event) => {
        ...
        else if (action === "levelup-attr" && id) {
    ...
    modal.addEventListener("mouseover", (event) => {
        ...
        const target = event.target;
        const equipButton = target.closest("button[data-action='inv-equip']");
        if (!equipButton) {
            return;
        }
...
function getViewport(playerPos, mapWidth, mapHeight, tileSize, canvas) {
...
function drawDungeon() {
    ...
    const view = getViewport(activeRun.player.position, activeRun.world.width, activeRun.world.height, tile, ui.canvas);
    const screenPos = (worldX, worldY) => ({
        x: (worldX - view.left) * tile,
        y: (worldY - view.top) * tile,
    });
    const inViewport = (worldX, worldY) => {
        const col = worldX - view.left;
        const row = worldY - view.top;
        return col >= 0 && col < view.cols && row >= 0 && row < view.rows;
    };
    const drawFog = (worldX, worldY) => {
        if (revealAll) {
            return;
        }
    ...
    const drawWorldSprite = (sheet, spriteId, worldX, worldY) => {
    ...
    const drawTileAccent = (x, y, roomAttrs, visible) => {
        ...
        if ((room.attrs & ROOM_SHOP) === ROOM_SHOP) {
            ...
            if (spritesReady) {
                ...
                for (const prop of props) {
                    ...
                    drawSheetSprite(context, "props", prop.sprite, (tx - view.left) * tile, (ty - view.top) * tile, tile);
                }
            }
            else {
    ...
    for (const chest of activeRun.chests) {
        ...
        const py = row * tile;
        if (spritesReady) {
            drawSheetSprite(context, "items", chest.chest.empty() ? SPRITE_IDS.items.chestOpen : SPRITE_IDS.items.chestClosed, px, py, tile);
        }
        else {
...
function renderStats() {
    ...
    let threatLabel = EN.ui.threat.unknown;
    if (inShop) {
    ...
    `;
        const gearDetails = ui.stats.querySelector("details[data-gear]");
        if (gearDetails) {
...
function overlayRenderKey(activeRun) {
    if (combatFx) {
...
function renderOverlay() {
    ...
    if (combatFx) {
        ...
        const knockoutMoment = shown
            .filter((moment) => moment.type === "text")
    ...
    if (run.overlay.type === "inventory") {
        ...
        const rows = activeRun.inventoryItems().map(({ item, action }) => {
            const actionButton = action === "equip"
...
function debugMoveResult(activeRun, steps, ok, reason) {
    return {
        ok,
        steps,
...
function installDebugBridge() {
    ...
    const roomSafeTile = (activeRun, room) => {
    ...
    const moveTo = (activeRun, target, opts) => {
        if (activeRun.overlay.type !== "none") {
            return debugMoveResult(activeRun, 0, false, `overlay:${activeRun.overlay.type}`);
        }
        if (activeRun.state !== "playing") {
            return debugMoveResult(activeRun, 0, false, `state:${activeRun.state}`);
        }
        const blocked = new Set();
        if (!opts?.ignoreMobs) {
            for (const mob of activeRun.mobs) {
        ...
        if (path.length === 0 && (activeRun.player.position.x !== target.x || activeRun.player.position.y !== target.y)) {
            return debugMoveResult(activeRun, 0, false, "no-path");
        }
        let steps = 0;
        for (const step of path) {
            ...
            steps += 1;
            if (activeRun.overlay.type !== "none") {
                return debugMoveResult(activeRun, steps, true);
            }
            if (activeRun.state !== "playing") {
                return debugMoveResult(activeRun, steps, false, `state:${activeRun.state}`);
            }
        }
        return debugMoveResult(activeRun, steps, true);
    };
    window.__surviveDebug = {
        ...
        moveBy: (dx, dy) => {
            ...
            const activeRun = run;
            activeRun.movePlayer(dx, dy);
            return debugMoveResult(activeRun, 1, true);
        },
        moveTo: (x, y, opts) => {
            if (!run) {
                return {
        ...
        moveToRoom: (kind) => {
            ...
            if (!room) {
                return debugMoveResult(activeRun, 0, false, "room-not-found");
            }
        ...
        moveToMob: (mobId) => {
            ...
            const target = mobId
                ...
                : alive
                    .slice()
                    .sort((a, b) => {
                    const da = Math.abs(a.creature.position.x - activeRun.player.position.x) + Math.abs(a.creature.position.y - activeRun.player.position.y);
                    const db = Math.abs(b.creature.position.x - activeRun.player.position.x) + Math.abs(b.creature.position.y - activeRun.player.position.y);
                    return da - db;
                })[0];
            if (!target) {
                return debugMoveResult(activeRun, 0, false, "mob-not-found");
            }
        ...
        moveToChest: (chestId) => {
            ...
            if (!target) {
                return debugMoveResult(activeRun, 0, false, "chest-not-found");
            }
            return moveTo(activeRun, { x: target.chest.x, y: target.chest.y }, { ignoreChests: true });
        },
        resolveCombat: (action = "normal", maxRounds = 24) => {
            if (!run) {
                return { rounds: 0, state: "none", overlay: "none" };
            }
            let rounds = 0;
            while (run.overlay.type === "battle" && run.state === "playing" && rounds < maxRounds) {
                const result = run.performCombat(action);
            ...
            return { rounds, state: run.state, overlay: run.overlay.type };
        },
        openInventory: () => {
            run?.openInventory();
        },
        ...
        chooseBossReward: (kind = "none", choiceId) => {
            run?.chooseBossReward(kind, choiceId);
        },
        claimShopReward: (choiceId) => {
            run?.claimShopReward(choiceId);
        },
        allocateLevelUp: (attr) => {
            run?.allocateLevelUp(attr);
        },
        canOpenShop: () => (run ? run.canOpenShop() : false),
        grantRegressionPower: () => {
            if (!run) {
                return;
            }
            const activeRun = run;
        ...
        exportSaveToken: () => {
            ...
            return persistRunSave(run);
        },
        importSaveToken: (token) => tryLoadRunFromTokenInput(token),
    };
}
installDebugBridge();
