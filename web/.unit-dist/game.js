import { AI_WANDER_MOVE_ROLL_MIN, AI_WANDER_ROLL_MAX, AI_MOVE_MS, BANDAGES_HEAL_HP, BOSS_CON_BONUS_BASE, BOSS_DEX_BONUS_BASE, BOSS_DEX_BONUS_FLOOR_DIVISOR, BOSS_CHALLENGE_LEVEL, BOSS_HP_SCALE_PER_FLOOR, BOSS_STR_BONUS_BASE, BANDAGES_GOLD_VALUE, CHEST_ARMOR_ROLL_MIN, CHEST_GOLD_DICE, CHEST_GOLD_ROLL_MIN, CHEST_HIGH_TIER_LEVEL, CHEST_HUGE_POTION_ROLL_MIN, CHEST_LARGE_OR_HEALTH_ROLL_MIN, CHEST_MID_TIER_LEVEL, CHEST_TIERED_POTION_ROLL_MIN, CHEST_WEAPON_ROLL_MIN, COLOSSAL_HEALTH_POTION_HEAL_HP, COLOSSAL_HEALTH_POTION_GOLD_VALUE, CREATURE_XP_MULTIPLIER, DANGER_WARNING_LEVEL_GAP, FLOOR_CHEST_BASE, FLOOR_CHEST_CAPACITY, FLOOR_CHEST_PER_FLOOR, FLOOR_DEPTH_SCALE_PER_FLOOR, FLOOR_MOB_BASE, FLOOR_MOB_PER_FLOOR, FLOOR_SCALE_MINIMUM, FLOOR_TRANSITION_HEAL_MINIMUM, FLOOR_TRANSITION_HEAL_RATIO, HEALTH_POTION_HEAL_HP, HEALTH_POTION_GOLD_VALUE, HUGE_HEALTH_POTION_HEAL_HP, HUGE_HEALTH_POTION_GOLD_VALUE, INSTAHEAL_GOLD_VALUE, ITEM_ARMOR_CHALLENGE_ROLL_MAX, ITEM_ARMOR_CHALLENGE_ROLL_MIN, ITEM_ARMOR_VALUE_ATTACK_WEIGHT, ITEM_ARMOR_VALUE_DEFENSE_WEIGHT, ITEM_DEFAULT_VALUE_MULTIPLIER, ITEM_POOL_GENERATION_COUNT, ITEM_SPECIAL_ROLL_MIN, ITEM_SPECIAL_VALUE_MULTIPLIER, ITEM_WEAPON_CHALLENGE_ROLL_MAX, ITEM_WEAPON_CHALLENGE_ROLL_MIN, ITEM_WEAPON_MAX_DAMAGE_ROLL_MAX, ITEM_WEAPON_MAX_DAMAGE_ROLL_MIN, ITEM_WEAPON_VALUE_DIE_MULTIPLIER, ITEM_WEAPON_VALUE_DIE_SIDES, LARGE_HEALTH_POTION_HEAL_HP, LARGE_HEALTH_POTION_GOLD_VALUE, LOG_LIMIT, MAXIMUM_CHALLENGE_LEVEL, PLAYER_CONSTITUTION_BONUS, PLAYER_START_X, PLAYER_START_Y, SHOP_CLUTTER_DENSITY_DIVISOR, SHOP_DISCOUNT_FOR_CHARISMA, SHOP_REWARD_FALLBACK_GOLD, SHOP_REWARD_HEAL_FALLBACK_RATIO, SHOP_SERVICE_COST_BONUS_POINT, SHOP_SERVICE_COST_REMOVE_GAMBIT, SHOP_SERVICE_COST_REMOVE_PERK, SHOP_TOP_ARMORS, SHOP_TOP_WEAPONS, STARTING_ATTRS, STARTING_BANDAGE_COUNT, STARTING_WEAPON_ATTACK_BONUS, STARTING_WEAPON_CRITICAL_MULTIPLIER, STARTING_WEAPON_CRITICAL_RANGE, STARTING_WEAPON_DAMAGE_DICE, STARTING_WEAPON_DEFENSE_BONUS, SURPRISE_PROTECTION_ATTACK_MULTIPLIER, SURPRISE_PROTECTION_FLEE_BONUS, } from "./constants";
import { AttributeSet } from "./attributes";
import { Combat } from "./combat";
import { Creature } from "./creature";
import { Dice } from "./dice";
import { Armor, Buff, Chest, Container, deserializeItem, Gold, InstantEffectItem, serializeItem, setNextItemId, getNextItemId, Poison, Weapon, WieldableItem, } from "./items";
...
const ATTRIBUTE_ORDER = ["str", "dex", "con", "int", "wis", "chr"];
const WIELD_SLOTS = ["head", "chest", "arms", "hands", "legs", "feet"];
...
const BUILD_CHOICE_BY_ID = new Map([...PERK_POOL, ...GAMBIT_POOL].map((choice) => [choice.id, choice]));
...
export class DungeonRun {
    ...
    shopStock = [];
    shopClutter = [];
    roomThreatById = new Map();
    warnedDangerRooms = new Set();
    dangerProtectionArmedRooms = new Set();
    activePerks = [];
    activeGambits = [];
    pendingBossRewards = null;
    pendingShopRewards = null;
    shopRewardClaimedFloors = new Set();
    constructor(seedInput) {
        ...
        this.player = new Creature(EN.game.names.player, { x: PLAYER_START_X, y: PLAYER_START_Y }, new AttributeSet(STARTING_ATTRS));
    ...
    preparePlayerGear() {
        const fists = new Weapon(EN.game.startingGear.fists, STARTING_WEAPON_CRITICAL_RANGE, STARTING_WEAPON_CRITICAL_MULTIPLIER, STARTING_WEAPON_ATTACK_BONUS, STARTING_WEAPON_DEFENSE_BONUS, STARTING_WEAPON_DAMAGE_DICE);
    ...
    generateItemPools() {
        ...
        for (let i = 0; i < ITEM_POOL_GENERATION_COUNT; i += 1) {
            ...
            const mountpoint = this.rng.choice(["head", "chest", "arms", "hands", "legs", "feet"]);
            const name = this.nameGenerator.generateName(mountpoint, special);
            if (mountpoint === "hands") {
                const weapon = createWeapon(name, this.dice.roll(ITEM_WEAPON_MAX_DAMAGE_ROLL_MIN, ITEM_WEAPON_MAX_DAMAGE_ROLL_MAX), this.dice.roll(ITEM_WEAPON_CHALLENGE_ROLL_MIN, ITEM_WEAPON_CHALLENGE_ROLL_MAX), this.rng);
                weapon.value = Math.floor(((this.dice.roll(1, ITEM_WEAPON_VALUE_DIE_SIDES) * ITEM_WEAPON_VALUE_DIE_MULTIPLIER)
                    + weapon.getAttackBonus()
                    + weapon.getDefenseBonus()) * valueMultiplier);
                weapons.push(weapon);
            }
            else {
                ...
                armor.value = Math.floor(((armor.getAttackBonus() * ITEM_ARMOR_VALUE_ATTACK_WEIGHT)
                    + (armor.getDefenseBonus() * ITEM_ARMOR_VALUE_DEFENSE_WEIGHT)) * valueMultiplier);
                armors.push(armor);
            }
        }
    ...
    populateDungeon() {
        ...
        for (const room of this.world.rooms) {
            ...
            const chestCount = this.dice.roll(1, Math.max(1, Math.ceil((FLOOR_CHEST_BASE + (this.floor * FLOOR_CHEST_PER_FLOOR)) * scaled)));
    ...
    removeBuildChoiceEffect(choice) {
        this.player.attackBonus -= choice.attackBonus ?? 0;
        this.player.defenseBonus -= choice.defenseBonus ?? 0;
        if (choice.attributes) {
            for (const attr of ATTRIBUTE_ORDER) {
    ...
    findRetreatPosition(battleRoomId, fallback) {
        ...
        const frontier = [clonePos(fallback)];
        const seen = new Set([posKey(fallback)]);
        let cursor = 0;
        ...
        this.stats.inventoryValue =
            this.player.inventory.items().reduce((acc, item) => acc + item.value, 0)
                + Object.values(this.player.wieldpoints)
                    .filter((item) => item instanceof WieldableItem)
    ...
    chooseBossReward(kind, choiceId) {
        ...
        this.log(EN.game.logs.selectedBuildChoice(picked.kind === "perk" ? EN.game.labels.perk : EN.game.labels.gambit, picked.name), "success");
        this.pendingBossRewards = null;
        this.advanceToNextFloor();
    }
    ...
    static hydrateCreature(target, saved) {
    ...
    static fromSaveData(saved) {
        ...
        run.world = new WorldMap(saved.world.width, saved.world.height, saved.world.rooms.map((room) => ({ ...room })), worldCells);
